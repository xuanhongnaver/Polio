...
; (function ($) {
    ...
    var headerFixed = function () {
        ...
        if ($('body').hasClass('Portfolio')) {
            ...
            var hd_submenu = $('#mainnav-mobi').height();
            $(window).on('load scroll', function () {
                if ($(window).scrollTop() > hd_submenu + 30) {
                    $('#mainnav-mobi').addClass('downscrolled');
                } else {
                    $('#mainnav-mobi').removeClass('downscrolled');
                }
                if ($(window).scrollTop() > 300) {
                    $('#mainnav-mobi').addClass('upscrolled');
                } else {
                    $('#mainnav-mobi').removeClass('upscrolled');
                }
            })
        }
    }
    ...
    var scrollBtn = function () {
        $('.tf-scroll').on('click', function () {
            ...
            if (anchor) {
                if ($('.' + anchor).length > 0) {
                    if ($('.Portfolio').length > 0 && largeScreen) {
                        headerHeight = headerHeight;
                    } else {
                        headerHeight = 0;
                    }
                    var target = $('.' + anchor).offset().top - headerHeight;
                    $('html,body').animate({ scrollTop: target }, 3000, 'easeInOutExpo');
                }
            }
            return false;
        })
    ...
    var scroll_tf = function () {
        $(function () {
            $('.tf-scroll345678').click(function () {
                $('html, body').animate({ scrollTop: $('div.bottom').offset().top }, 'slow');
                return false;
            });
        });
    };
    ...
    var donatProgress = function () {
        if ($(document.body).hasClass("Portfolio")) {
            var a = 0;
            $(window).scroll(function () {
                var oTop = $(".expertise-left").offset().top - window.innerHeight;
                if (a == 0 && $(window).scrollTop() > oTop) {
                    if ($().countTo) {
                        $('.skill-bar').each(function () {
                            $(this).find('.skill-content').animate({
                                width: $(this).attr('data-percentage')
                            }, 2000);

                            $(this).find('.skill-number-mark').animate(
